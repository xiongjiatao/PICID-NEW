"""CLI for generating scaling law plots from W&B experiment data."""
...
ALL_PROJECTS = [
    ...
    "16_02_2026_scaling_blockwise_phme20_prognostics_raw",
    # MZVAV
    "16_04_2026_scaling_mzvav_diagnostics_",
    "16_04_2026_scaling_blockwise_mzvav_diagnostics_",
    # Unibo
    "16_04_2026_scaling_blockwise_unibo_prognostics_combined",
    "16_04_2026_scaling_unibo_prognostics_combined",
    # Concepts n_cmapss_ds02
    "16_04_2026_scaling_concepts_n_cmapss_ds02_prognostics_",
    "16_04_2026_scaling_blockwise_concepts_n_cmapss_ds02_prognostics_",
]
...
def detect_dataset_type(project_name: str) -> str:
    ...
    if "phme20" in project_name:
        return "phme20"
    if "mzvav" in project_name:
        return "mzvav"
    raise ValueError(
        f"Cannot detect dataset type for project '{project_name}'. "
        f"Expected one of: unibo/nb14, n_cmapss_ds02, n_cmapss, phme20, mzvav."
    )
...
def prepare_df_long(df: pd.DataFrame, dataset_type: str) -> pd.DataFrame:
    ...
    # Merge legacy column names into normalized ones when both exist
    ...
    # For multi-source n_cmapss, attach source label and keep only second source
    ...
    # Carry model_type through the melt so we can append it to model._target_ afterwards
    ...
    # Append " (linear)" / " (exponential)" to model._target_ — mirrors preprocess.py:66-74
...
def _select_topk_models(
    plot_df: pd.DataFrame,
    active_rename: dict,
    metric: str,
    topk: int,
) -> dict:
    """Return a subset of active_rename with only the top-k models by mean metric."""
    higher_is_better = "f1" in metric
    metric_df = plot_df[
        plot_df["model._target_"].isin(active_rename.keys())
        & (plot_df["metric"] == metric)
    ]
    mean_per_model = (
        metric_df.groupby("model._target_")["value"].mean()
    )
    ranked = mean_per_model.sort_values(ascending=not higher_is_better)
    top_keys = set(ranked.head(topk).index)
    return {k: v for k, v in active_rename.items() if k in top_keys}
...
def generate_plot(
    ...
    if topk is not None:
        active_rename = _select_topk_models(plot_df, active_rename, metric, topk)
        if not active_rename:
            raise ValueError(f"--topk {topk} filtered out all models.")

    # Fix legend order and colors across all plots
    active_rename = dict(
        sorted(
            active_rename.items(),
    ...
    with sns.plotting_context("paper", font_scale=2, rc={"font.family": "Helvetica"}):
        ...
        sns.lineplot(
            ...
            x="task_definition.subset_ratio",
            y="value",
            hue="model._target_",
        ...
        if not no_title:
            ax.set_title(
                f"Performance vs. Randomly Selected Training Data Size [Dataset: {dataset_name}]"
            )
        ax.set_xlabel("Fraction of the Data")
        ...
        blockwise = "_blockwise" if "blockwise" in project_name else ""
        date_match = re.search(r"\d{2}_\d{2}_\d{4}", project_name)
        date_prefix = f"{date_match.group()}_" if date_match else ""
        topk_suffix = f"_top{topk}" if topk is not None else ""
        output_path = (
            Path(output_dir)
            / f"{date_prefix}scaling_laws_{dataset_name}{blockwise}_{metric.split('/')[1]}{topk_suffix}.pdf"
        )
...
def display_run_counts_table(df_long: pd.DataFrame, dataset_type: str) -> None:
    ...
    counts = (
        df_metric.groupby(["model._target_", "task_definition.subset_ratio"])
        .size()
        .unstack("task_definition.subset_ratio")
        .sort_index(axis=1)
    )
    counts.index = counts.index.map(lambda x: MODEL_RENAME.get(x, x.split(".")[-1]))

    subset_cols = [f"{c:.4g}" for c in counts.columns]
    table = Table(
        title=f"Runs per model × subset ratio  (metric: {metric})", show_lines=True
    )
...
def run_project(project: str, user: str, output_dir: str, no_summary: bool, errorbar: str | tuple | None = "sd", no_title: bool = False, topk: int | None = None) -> Path:
    ...
    with Progress(
        ...
        output_path = generate_plot(df_long, dataset_type, output_dir, project, errorbar=errorbar, no_title=no_title, topk=topk)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate scaling law plots from W&B experiment data.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python scaling_laws_plots.py -p 16_04_2026_scaling_blockwise_mzvav_diagnostics_\n"
            "  python scaling_laws_plots.py -p n_cmapss_scaling -u my-wandb-user -o ./out\n"
            "  python scaling_laws_plots.py --all\n"
        ),
    )
    ...
    parser.add_argument(
        ...
        metavar="K",
        help=(
            "Only plot the top K models ranked by their mean metric across all subset ratios. "
            "For F1 (higher-is-better) the top K highest are kept; "
            "for MAE/RMSE (lower-is-better) the top K lowest are kept."
        ),
    )
    ...
    for project in projects:
        try:
            run_project(project, args.user, args.output_dir, args.no_summary, errorbar=_parse_errorbar(args.errorbar), no_title=args.no_title, topk=args.topk)
