#!/usr/bin/env python3
"""Tutorial: UNIBO21 Powertools batteries with Bosello-style predefined splits."""
...
def get_unibo21_bosello_loader_class() -> type[Any]:
    """Return :class:`UNIBO21Loader`; patch in tests to avoid real dataset IO."""
    from picid.data.datasources.unibo.loader import UNIBO21Loader

    return UNIBO21Loader
...
def load_unibo21_bosello_yaml() -> dict[str, str]:
    """Load ``configs/datasource/unibo21_bosello.yaml`` top-level keys."""
    repo_root = Path(__file__).resolve().parents[2]
    path = repo_root / "configs" / "datasource" / "unibo21_bosello.yaml"
...
def _build_unibo21_bosello_notes(container: Any) -> str:
    """Domain notes: capacity-threshold annotations and group-wise split design."""
    bullets = [
        "capacity-threshold — Unit comments in ``picid.data.datasources.unibo.loader`` "
        "record minimum observed capacity near end of life; exclusions (e.g. Bosello "
        "et al. 2023) align train/val/test with cells that meet the protocol thresholds.",
        "group-split — Batteries are grouped by chemistry/condition; validation skips "
        "groups with only two units (one train / one test), matching the Bosello-style "
        "UNIBO partition encoded in ``train_names``, ``val_names``, and ``test_names``.",
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load UNIBO21 with UNIBO21Loader (Bosello splits) and print the standard "
    ...
    add_data_dir_argument(
        ...
        help=(
            ...
            "``configs/datasource/unibo21_bosello.yaml``."
        ),
    )
...
def main(*, data_dir_cli: str | None = None) -> None:
    ...
    cfg = load_unibo21_bosello_yaml()
    data_name = cfg.get("data_name", "UNIBO21")
    task_mode = cfg.get("task_mode", "SOC")

    Loader = get_unibo21_bosello_loader_class()
    loader = Loader(
    ...
    if not features_by_split:
        raise RuntimeError(
            "UNIBO21Loader returned no features; check data_dir and dataset layout."
        )
    ...
    loader_config_body = "\n".join(
        [
            ...
            "config: configs/datasource/unibo21_bosello.yaml",
        ]
    )
    ...
    dataset_notes_body = format_dataset_specific_notes(
        _build_unibo21_bosello_notes(container)
    )
    ...
    print("UNIBO21 BOSSELLO TUTORIAL")
    print()
    print(report)
