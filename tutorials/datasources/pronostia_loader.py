#!/usr/bin/env python3
"""Tutorial: compose PRONOSTIA (IEEE 2012) datasource config, load, and print a report."""
...
def format_operating_condition_coverage(container: Any) -> str:
    """Per-split counts of units under each PRONOSTIA operating condition (1/2/3)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compose PRONOSTIA (IEEE 2012 raw layout) datasource config via Hydra, "
            "load, and print a standard datasource tutorial report."
        )
    )
    add_data_dir_argument(
        parser,
        help=(
            "Override datasets root; raw data is expected under "
            "``${paths.data_dir}/pronostia_ieee2012`` (see configs/datasource/pronostia.yaml)."
        ),
    )
    add_no_download_argument(parser)
    return parser.parse_args()


def _compose_pronostia_config(
    *,
    ...
    if no_download:
        overrides.append("datasource.download_if_missing=false")
...
def _format_loader_config_summary(cfg: DictConfig, loader: Any) -> str:
    ds = cfg.datasource
    cfg_path = _CONFIG_DIR / "datasource" / "pronostia.yaml"
    fold = OmegaConf.select(ds, "fold_id")
    sample_range = OmegaConf.select(ds, "sample_range_size")
    dl = OmegaConf.select(ds, "download_if_missing")
    return "\n".join(
        [
            f"config_file: {cfg_path}",
            f"data_name: {getattr(loader, 'data_name', '')}",
            f"task_mode: {getattr(loader, 'task_mode', '')}",
            f"data_dir (resolved): {getattr(loader, 'data_dir', '')}",
            f"split_mode: {getattr(loader, 'split_mode', '')}",
            f"fold_id (CV protocol): {fold!r}",
            f"sample_range_size: {sample_range!r}",
            f"download_if_missing: {dl!r}",
        ]
    )
...
def _build_dataset_notes(loader: Any, container: Any) -> str:
    lines = [
        "PRONOSTIA (IEEE PHM 2012 Data Challenge) via PronostiaIEEE2012Loader: "
        "Kaggle-style Learning_set + Full_Test_Set layout, two vibration channels "
        "per 2560-sample window, RUL target aligned with the PHMD-backed loader.",
        "",
        "split_mode selects the evaluation family: "
        "'in_domain' runs 5-fold CV mixing all three operating conditions; "
        ...
        "(see split_assignments_by_mode in configs/datasource/pronostia.yaml).",
        "",
        "Without local files, the loader can sparse-clone the public mirror "
        "(requires git). Use --no-download to require an on-disk layout instead.",
    ]
    records = flatten_records(container)
...
def main() -> None:
    """Compose pronostia datasource config, load, and print reports."""
    args = parse_args()
    cfg = _compose_pronostia_config(
    ...
    try:
        ...
        if not features_by_split:
            raise RuntimeError(
                "PronostiaIEEE2012Loader returned no features; check data_dir "
                "and configs/datasource/pronostia.yaml."
            )

        print("PRONOSTIA DATASOURCE TUTORIAL (IEEE 2012 raw layout)")
