...
# ==========================================================================
# Feature ablation (diagnostics): hsf15 is the only diagnostics dataset whose
# "default" transforms concatenate TimeStatsTransform (temporal) +
# SpectralStatsTransform (spectral) into "features". This script reruns the
# same hsf15 diagnostics experiments but overrides the `transforms` config
# group with a temporal-only pipeline (the spectral block dropped, time-domain
# stats fed straight into "features"), to show how much of the default
# pipeline's performance comes from the spectral features. Mirrors
# scripts/rebuttal/nb14_unibo_xjtu_time_stats_ablation.sh for prognostics.
# ==========================================================================

max_epochs=200
check_val_every_n_epoch=1
device=0
wandb_log_folder="rebuttal_time_stats_ablation_hsf15"
# Set DEBUG_SKIP to any non-empty value to log runs without executing commands
DEBUG_SKIP="${DEBUG_SKIP:-}"

# dataset_key|subexp|task_type|transforms_override
# subexp is empty for hsf15 (exp_name is dataset_key/task_type/model, no subexp
# level); transforms_override swaps in the temporal-only pipeline on top.
datasets=(
  "hsf15/accumulator||diagnostics|hsf15/statistics"
  "hsf15/cooler||diagnostics|hsf15/statistics"
  "hsf15/pump||diagnostics|hsf15/statistics"
  "hsf15/valve||diagnostics|hsf15/statistics"
)
    ...
    for model_name in "${model_names[@]}"; do
        for batch_size in "${BATCH_SIZES[@]}"; do
                    for seed in "${SEEDS[@]}"; do
                        ...
                        IFS='|' read -r dataset_key subexp task_type transforms_override <<< "$dataset"
                        exp_name="$(build_exp_name "$dataset_key" "$task_type" "$subexp" "$model_name")"
                        dataset_run_name="${dataset_key//\//_}"
                        subexp_run_name="${subexp//\//_}"
                        run_name=$(printf "%03d_%s_%s_%s_%s_timestats_%s" "$run_counter" "$dataset_run_name" "$model_name" "$task_type" "$subexp_run_name" "$seed" )
                        final_log_folder="${wandb_log_folder}_${dataset_run_name}"_"${task_type}"_"${subexp_run_name}"
