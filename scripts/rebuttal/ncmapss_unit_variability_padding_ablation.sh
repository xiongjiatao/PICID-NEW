...
# ==========================================================================
# Padding ablation: pairs with ncmapss_unit_variability_core_baselines.sh.
# Sweeps task_definition.padding_left_flag (True vs False) using the
# dataset's original (default) training units, to show that left-padding
# (which keeps train/val/test sequence lengths consistent regardless of how
# short an individual unit's history is) matters: without it, units shorter
# than seq_len+pred_len+pred_offset are silently dropped by the sequencer
...
device=0
wandb_log_folder="rebuttal_ncmapss_padding_ablation"
...
# The ablation axis: with padding, short units are left-padded to a
# consistent length; without it, the sequencer enforces strict in-bounds
# windows only and drops units that are too short outright.
padding_flags=(
    "True"
    "False"
)
...
for seed in "${SEEDS[@]}"; do
    for dataset in "${datasets[@]}"; do
        for model_name in "${model_names[@]}"; do
            for padding_left_flag in "${padding_flags[@]}"; do
                        ...
                        pad_label="pad_${padding_left_flag}"
                        run_name=$(printf "%03d_%s_%s_%s_%s_%s_%s" "$run_counter" "$dataset_run_name" "$model_name" "$task_type" "$subexp" "$pad_label" "$seed" )
                        ...
                        command_to_run=(
                            ...
                            task_definition.padding_left_flag=${padding_left_flag} \
                            experiment_group=${pad_label} \
                            seed=${seed}
                        )
