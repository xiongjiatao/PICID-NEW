...
# ==========================================================================
# Pre-caching run for scripts/rebuttal/hsf15_time_stats_ablation.sh: builds
# the preprocessing cache for the temporal-only transforms=hsf15/statistics
# pipeline once per hsf15 unit, single-threaded, before the full model sweep
# launches many parallel jobs against it.
# ==========================================================================
...
# Must match scripts/rebuttal/hsf15_time_stats_ablation.sh exactly.
datasets=(
            ...
            for seq_len in "${SEQ_LEN[@]}"; do
                for max_learning_rate in "${MAX_LEARNING_RATE[@]}"; do
                    for seed in "${SEEDS[@]}"; do
                        ...
                        run_name=$(printf "%03d_%s_%s_%s_%s_%s" "$run_counter" "$dataset_run_name" "$model_name" "$task_type" "$subexp_run_name" "$seed" )
