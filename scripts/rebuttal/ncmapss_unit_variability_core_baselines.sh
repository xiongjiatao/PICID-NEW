...
device=0
wandb_log_folder="rebuttal_ncmapss_unit_variability"
...
for seed in "${SEEDS[@]}"; do
    for dataset in "${datasets[@]}"; do
        for model_name in "${model_names[@]}"; do
            for train_units in "${train_unit_sets[@]}"; do
                        ...
                        unit_label="${train_units//,/_}"
                        run_name=$(printf "%03d_%s_%s_%s_%s_units_%s_%s" "$run_counter" "$dataset_run_name" "$model_name" "$task_type" "$subexp" "$unit_label" "$seed" )
                        ...
                        command_to_run=(
                            ...
                            datasource.train1.load_arguments.units=[${train_units}] \
                            experiment_group=units_${unit_label} \
                            seed=${seed}
                        )
