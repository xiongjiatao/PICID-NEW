...
class ConvBlock(nn.Module):
    ...
    def forward(self, x):
        x = rearrange(
            x, "batch (sl len) d_model -> (batch sl) d_model len", sl=self.split_length
        )
        x = self.conv_forward(x)
        x = rearrange(
            x, "(batch sl) d_model len -> batch (sl len) d_model", sl=self.split_length
        )
        return x
