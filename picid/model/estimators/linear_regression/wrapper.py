"""Canonical wrapper for the linear-regression forecasting baseline."""

from picid.model.definitions import FORECASTING_TASKS
from picid.model.adapters.base import AbstractFeedForwardWrapper

from .model import LinearRegressionModelBaseline


class LinearRegressionModelWrapper(AbstractFeedForwardWrapper):
    """
    Wrap the linear-regression baseline in the model interface.
    ...
    **kwargs : Any
    ...
    def __init__(self, task_type: str, **kwargs):
        """
        Initialize the linear-regression wrapper for forecasting tasks.
        ...
        **kwargs : Any
        ...
        if task_type not in self.supported_types:
            raise ValueError(
                f"Task {task_type} not supported for LinearRegressionModelWrapper."
            )

        self.task_type = task_type
        backbone = LinearRegressionModelBaseline(**kwargs)
        super().__init__(backbone=backbone, **kwargs)

    def forward(self, batch):
        """
        Run the linear-regression baseline on one batch.
