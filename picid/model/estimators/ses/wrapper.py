"""Canonical wrapper for the simple exponential smoothing baseline."""

from picid.model.definitions import FORECASTING_TASKS
from picid.model.adapters.base import AbstractFeedForwardWrapper

from .model import SESModelBaseline


class SESModelWrapper(AbstractFeedForwardWrapper):
    """
    Wrap the SES baseline in the model interface.
    ...
    **kwargs : Any
    ...
    def __init__(self, task_type: str, **kwargs):
        """
        Initialize the SES wrapper for forecasting tasks.
        ...
        **kwargs : Any
        ...
        if task_type not in self.supported_types:
            raise ValueError(f"Task {task_type} not supported for SESModelWrapper.")

        self.task_type = task_type

        backbone = SESModelBaseline(**kwargs)
        super().__init__(backbone=backbone, **kwargs)

    def forward(self, batch):
        """
        Run the SES baseline on one batch.
