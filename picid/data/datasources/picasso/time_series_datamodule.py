# THIS IS THE REFERENCE IMPLEMENTATION USED FOR picasso_datasource
...
class ContextBatchDatasetEnhanced(Dataset):
    ...
    def __init__(
        ...
        context_col_mapping_past,
        context_col_mapping_future,
        ts_in,
        ts_out,
        lags_targets,
        step,
        raw_data=None,
        out_features_mask=None,
    ):
    ...
    def _get_single_item(self, idx):
        ...
        for col_name, steps in self.future_context_steps.items():
            ...
            col_data = self.Context_future[
                future_start:future_end, col_idx : col_idx + 1
            ]
        ...
        if x_c_future_list or x_c_past_list:
            x_c = torch.cat([*x_c_past_list, *x_c_future_list])
        else:
        ...
        if (
            hasattr(self, "Context_future")
            and self.Context_future.shape[0] > future_start + self.ts_out
        ):
